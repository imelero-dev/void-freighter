...
// absolute throttle slider with a spring-loaded BOOST band, FIRE/MSL hold
// buttons and two collapsible NAV/SYS action menus in the top-left corner
// (ROLL holds stay at the bottom corners and become a strafe stick on drag).
...
export class TouchControls {
  ...
  private build(): HTMLElement {
    ...
    // top-left: two collapsed menus keep 13 actions off the flight view
    ...
    menus.appendChild(this.menuButton('SYS ▾', [
      ...
      ['ASSIST', 'toggleAssist'], ['LIGHTS', 'toggleLights'], ['CAM', 'toggleCamera'],
    ]));
    ...
    // right thumb: primary fire (hold) + missile (tap; right mouse equivalent)
    ...
    const sec = this.holdButton('MSL', (on) => {
      if (on && !this.input.uiMode) this.input.triggerMissile();
    });
    ...
    // turns the hold into a strafe mini-stick.
